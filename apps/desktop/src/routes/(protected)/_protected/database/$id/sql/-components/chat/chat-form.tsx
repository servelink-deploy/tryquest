...
export function ChatForm() {
  ...
  const handleSend = async (value: string) => {
    ...
    try {
      // Upload images to storage instead of converting to base64
      const uploadedImages = await Promise.all(
        cachedFiles.map(async (file) => {
          const base64Array = await getBase64FromFiles([file])
          ...
          try {
            // Upload to S3/R2 via API
            const { url } = await orpc.uploadImageToStorage({
              base64,
              contentType,
            })
            ...
            console.warn('Failed to upload image to storage, falling back to base64', uploadError)
      ...
      await chat.sendMessage({
        ...
        parts: [
          ...
          ...uploadedImages.map(({ url, contentType }) => ({
            type: 'file' as const,
            url,
