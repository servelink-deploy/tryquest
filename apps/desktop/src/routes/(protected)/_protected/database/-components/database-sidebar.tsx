...
function LastOpenedDatabase({ database, isExpanded }: { database: typeof databases.$inferSelect, isExpanded: boolean }) {
  ...
  return (
    <TooltipProvider>
      <Tooltip open={!isExpanded && undefined}>
        <TooltipTrigger asChild>
          <div className="relative group">
            ...
            <Link
              className={cn(classes(isActive), isExpanded && 'justify-start px-3 gap-2')}
              {...params}
            >
              {isExpanded ? (
                <>
                  <DatabaseIcon type={database.type} className="size-4 flex-shrink-0" />
...
function LastOpenedDatabases({ isExpanded }: { isExpanded: boolean }) {
  ...
  return filteredDatabases.map(database => <LastOpenedDatabase key={database.id} database={database} isExpanded={isExpanded} />)
}

function MainLinks({ isExpanded }: { isExpanded: boolean }) {
    ...
    <>
        ...
        <Tooltip open={!isExpanded && undefined}>
          <TooltipTrigger asChild>
            <Link
              ...
              className={cn(classes(isActiveSql), isExpanded && 'justify-start px-3 gap-2')}
            >
              <RiPlayLargeLine className="size-4 flex-shrink-0" />
              {isExpanded && <span className="text-sm whitespace-nowrap">SQL Runner</span>}
            </Link>
          </TooltipTrigger>
      ...
      <TooltipProvider>
        <Tooltip open={!isExpanded && undefined}>
          <TooltipTrigger asChild>
            <Link
              className={cn(classes(isActiveTables), isExpanded && 'justify-start px-3 gap-2')}
              {...route}
              onClick={() => {
                onTablesClick()
              }}
            >
              <RiTableLine className="size-4 flex-shrink-0" />
              {isExpanded && <span className="text-sm whitespace-nowrap">Tables</span>}
            </Link>
          </TooltipTrigger>
        ...
        <Tooltip open={!isExpanded && undefined}>
          <TooltipTrigger asChild>
            <Link
              ...
              params={{ id: database.id }}
              className={cn(classes(isActiveEnums), isExpanded && 'justify-start px-3 gap-2')}
            >
              <RiListUnordered className="size-4 flex-shrink-0" />
              {isExpanded && <span className="text-sm whitespace-nowrap">Enums</span>}
            </Link>
          </TooltipTrigger>
      ...
      <TooltipProvider>
        <Tooltip open={!isExpanded && undefined}>
          <TooltipTrigger asChild>
            <Link to="/database/$id/visualizer" params={{ id: database.id }} className={cn(classes(isActiveVisualizer), isExpanded && 'justify-start px-3 gap-2')}>
              <RiNodeTree className="size-4 flex-shrink-0" />
              {isExpanded && <span className="text-sm whitespace-nowrap">Visualizer</span>}
            </Link>
          </TooltipTrigger>
...
export function DatabaseSidebar({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <div 
      className={cn('flex flex-col transition-all duration-200 ease-in-out absolute left-0 top-0 h-full z-10 bg-background border-r', className)} 
      style={{ width: isExpanded ? '200px' : '64px' }}
      onMouseEnter={() => setIsExpanded(true)}
      onMouseLeave={() => setIsExpanded(false)}
      {...props}
    >
      <div className={cn('flex p-4 pb-0', isExpanded ? 'justify-start items-center gap-2' : 'flex-col justify-center items-center')}>
        <TooltipProvider>
          <Tooltip open={!isExpanded && undefined}>
            <TooltipTrigger asChild>
              <Link
              ...
              >
                <AppLogo className="size-6 text-primary flex-shrink-0" />
                {isExpanded && <span className="font-semibold whitespace-nowrap">Dashboard</span>}
              </Link>
            </TooltipTrigger>
      ...
      <ScrollArea className="relative p-4 flex flex-col flex-1 gap-2" style={{ alignItems: isExpanded ? 'stretch' : 'center' }}>
          ...
          <div className="flex w-full flex-col gap-1">
            ...
            {lastOpenedDatabases.length > 1 && (
              <>
                ...
                <LastOpenedDatabases isExpanded={isExpanded} />
              </>
            )}
          </div>
        </div>
